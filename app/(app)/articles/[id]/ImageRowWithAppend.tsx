...
export function ImageRowWithAppend({ articleId, imageUrls }: Props) {
  ...
  return (
    <div>
      {/* Horizontal image strip with + button */}
        ...
        {imageUrls.map((url, i) => (
          <a key={i} href={url} target="_blank" rel="noopener noreferrer" className="shrink-0">
            <img
              src={url}
              alt={`原图 ${i + 1}`}
              className="h-64 w-auto rounded-lg border object-cover hover:opacity-90 transition-opacity"
            />
          </a>
        ))}
        <button
      ...
      {open && (
        <div className="mt-3 border rounded-xl p-4 space-y-3 bg-muted">
          ...
          {items.length > 0 && (
            <div className={cn('grid gap-2', items.length === 1 ? 'grid-cols-2' : 'grid-cols-3')}>
              {items.map((item, i) => (
          ...
          {isProcessing && (
            <p className="text-sm text-muted-foreground flex items-center gap-1.5">
              ...
              {phase === 'processing' ? `正在处理第 ${processingIdx + 1} / ${items.length} 张…` : 'AI 识别文字中…'}
            </p>
          )}

          {phase === 'done' && (
            <p className="text-sm text-green-600 flex items-center gap-1.5">
              <CheckCircle2 className="h-4 w-4" />追加成功！
            </p>
          )}
